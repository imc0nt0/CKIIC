function Hangle(text) {
  ...
  const VOWELS = new Set(JUNGSUNG_LIST.flatMap(jung => 
    COMPLEX_JUNGSUNG_MAP[jung] ? COMPLEX_JUNGSUNG_MAP[jung] : [jung]
  ));

  const CONSONANTS = new Set([...CHOSUNG_LIST, ...JONGSUNG_LIST.filter(c => c)]);
